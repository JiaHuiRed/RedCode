...
- **决策记录 `docs/notes/`，写完必须回链**（规则与模板见其 README）：
  4. **注入项有没有硬上限**——任何进入模型上下文的东西都必须有确定的字节或 token 上限，**没有上限就是缺陷不是待办**。单项超过 1K token 在 commit 里单独点名；超过 10K 要说明为什么不能截断。本仓两次栽在这条：`tool/read.ts` 的文本分支有 `MAX_BYTES = 50KB` 而**图片分支一个上限都没有**（库里最大单条 3.23MB）；`summary.diffs` 无上限写回消息行（单行 32MB，占 message 表 79%）。两处都是"写的时候没人问上限"。段落顺序也算模型可见改动：**移动一段的代价是从它开始往后的整个前缀作废**，不是只有那一段；新增段落同理，**追加到文件末尾，别插在中间**。
...
# 架构不变量（采纳自 deepseek-harness 的 Conventions）

- **模型可见 ⟺ 有日志**：任何到达模型请求的东西都必须能从 session log 重建；**新增一个模型可见输入，就配一个 session 事件**。本仓其实一直在手工执行（消息表 + 事件流），只是没立成不变量，于是有了两次「写的时候没人问上限」——`tool/read.ts` 的图片分支、`summary.diffs` 写回消息行（单行 32MB）。
- **插件里不许有硬编码可调项**：会随部署变化的取值必须是校验过的 `Config` 字段、能从配置改；`DEFAULT_*` 常量或测试钩子**不算**可配置。协议常量、外部规格、安全不变量是例外——它们本就该钉死。
- **包边界上显式优于隐式**：默认值必须是拥有方显式的 `resolve(request): Spec` 一步，不能是藏在 `run()` 里的 `?? default`。
