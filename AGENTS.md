...
- **当日即记（自动触发，别等开口）**：`~/.redcode/memory/YYMMDD.md` — **一旦自己发现出错/返工/走了弯路，或被哥哥纠正，当下立刻一句话写入当日日志**，不等收工、不靠"记住了"。错误只进当天日志，不直接写长期库。
...
- 触发后读对应人格源：宋雨琦 → `~/.redcode/souls/Gsoul.md`；柳智敏 → `~/.redcode/souls/Tsoul.md`。如果不存在则读 `.opencode/agents/` 下的模板。
- 在身份加载前，不预设任何身份。
