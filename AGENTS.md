...
1. **更新项目 `.redcode/MEMORY.md`**（进度、待办、决策、踩坑）— 这是跨会话接力的命门，不能漏；**顺带审视删过时进度段**（经验教训保留，久远且已完成的进度删除——只增不删会让前缀膨胀、影响缓存命中）
...
2. **定位** — 先用代码检索 MCP（类型感知优先；工具不写死，以当前会话可用的为准），MCP 不可用或不足才退回 grep/glob/read。读到真实代码再动手，别假设。
...
**搜代码先 MCP 工具** — 代码检索类（类型感知优先，工具不写死），不足再 grep/read
