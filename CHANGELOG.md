...
- **会话页空白的真根因：后台节流让页面被判隐藏**（`packages/desktop/src/main/windows.ts`、`packages/app/src/pages/session/message-timeline.tsx`）：Electron 默认 `backgroundThrottling: true`，窗口被判定遮挡时页面进入 hidden；`virtua` 只在 scroll/resize 事件上重算可视区间、没有可见性感知，隐藏期间保留的旧测量区间在恢复显示后原样生效，视口内一条都不渲染（CDP 实测 hidden 时 DOM 项位于 y=26892…28332，而视口高仅 1231；同一时刻性能面板 FPS 132，可排除主线程阻塞）。关闭窗口后台节流让渲染链路不再被遮挡判定打断，并在恢复 `visibilitychange` 时对列表做一次 1px 滚动往返强制重算，作为不依赖主进程配置的兜底。
- **指令预算告警不再污染 TUI 渲染**（`packages/opencode/src/session/instruction.ts`）：`instruction_budget` 的来源跳过与超预算告警原先走 `Console.warn`，而 server 运行在 Worker 线程里、其 console 不受 TUI 主线程 console-hijack 保护，会经 stderr 继承直接打到终端并叠在输入框附近。改为 `Log.Default.warn`：诊断照旧落日志文件，仅 `--print-logs` 场景打屏，与 `worker.ts` 抑制 `MaxListenersExceededWarning` 同一处理方式。
- **会话页空白的一条触发条件：Chromium 原生窗口遮挡**（`packages/desktop/src/main/index.ts`、`packages/app/src/pages/session/message-timeline.tsx`）：窗口被其他窗口盖住时，Chromium 在 Windows 上的原生遮挡计算（feature `CalculateNativeWinOcclusion`）把页面判为 hidden，于是跳过布局、ResizeObserver 不再回调，`virtua` 的 `viewportSize` 永远停在 0（内部区间门 `S=false → [0,-1]`），可视区间恒为空 → 整片空白，且遮挡解除前不自愈（CDP 实测：`document.hidden=true`、ResizeObserver 回调数为 0、DOM 中唯一的行是 `visibility: hidden` 的占位项，而 `window.innerHeight` 与容器 `clientHeight` 都正常）。此前加的 `backgroundThrottling: false` 只关掉 Page Visibility 那一层——实测打开后 `document.hidden` 仍为 true，Electron 也没有为遮挡判定提供 `webPreferences` 开关——因此在主进程关掉该 feature；一次性对照实验：同一份最小 Electron 脚本，不关时被遮挡窗口 `document.hidden` 恒 `true`，关掉后恒 `false`。渲染侧保留兜底：`visibilitychange` 与窗口 `focus` 时若 `viewportSize` 仍为 0，用 keyed 重建让虚拟列表重新测量并重新 observe。**这一条只是触发条件之一** —— 第二轮审计确认「窗口在前台、焦点也在它身上仍然整片空白」另有独立成因（见下条），遮挡不构成统一解释。

- **会话页主时间线增加 native 渲染分支**（`packages/app/src/pages/session/message-timeline.tsx`）：外部第二轮审计指出「整片空白」已是**结构性失败模式**而非若干互不相关的小 bug —— 当前架构把「Virtua 的成功测量 ≈ 消息能否被用户看到」绑成了同一件事，viewportSize / visible range / offset / mount 任一环节异常都会退化成整个会话一行不渲染且没有降级；同时外层又拿虚拟 DOM 的几何状态（`scrollHeight` 判断是否继续拉历史、`getElementById` 判断跳转是否成功）驱动业务决策，等于把渲染暂态当成业务事实。按审计建议先取消「必须通过 Virtualizer 才能渲染」的单路径：加一条普通 DOM flow 作为可对照的第二条渲染路径，两种渲染共用同一份 `timelineRows` / `TimelineRowView` / 分页 / history loader，只换最外层容器；开关走 `localStorage`（`redcode:native-timeline`），刷新即切换，不碰用户配置。native 路径下行全部真实挂在 DOM 里，`seek()` 的锚点查找天然命中，也不必再要求「必须有 virtualizer」才算底部锚定。审计材料见 `.redcode/gui-render-audit-2.md`。
- **消息分页的并发请求不再被静默丢弃**（`packages/app/src/context/directory-sync.ts`）：`loadMessages` 与 `loadMore` 原先各有一道 `if (meta.loading[key]) return`，并发调用直接返回 `undefined`，调用方分不清「我的请求跑完了」和「我的请求根本没执行」，`session-history-loader` 的 `loadThrough` 因此只能用 60ms 等待 + 最多 8 次 stall 去猜 pager 何时可用 —— 这是轮次跳转「时灵时不灵」的直接原因。改为复用仓库既有的 `runInflight`：请求仍然只发一次，但每个调用方都能 await 到真实完成信号。
...
- **局域网绑定收拢密码闸门，网卡枚举不再打印不可达地址**（`packages/opencode/src/cli/network.ts`、`src/cli/cmd/run.ts`、`src/cli/cmd/web.ts`）：`serve`/`web` 走 `cli/network.ts` 的密码闸门，TUI 直连的 `--hostname` 分支却直调 `Server.listen`——同一个 `--hostname 0.0.0.0`，前者会拦或兜底注入默认密码，后者把 server 裸奔到局域网；现在 run.ts 也走 `resolveLanDefaultPassword` + `assertPasswordForExposure`。另外 web/run 两份重复的网卡枚举都只跳过 `172.*`，会把 link-local（`169.254.*`，手机不可达）和代理 TUN 常占的 RFC 2544 基准段（`198.18/198.19`）当作可访问地址打印，照着敲一个连不上一个；抽成 `getLanIPs()` / `isUsableLanAddress()` 后两处共用。
...
- **补上 `seed/tool/` 的同步链路**（`script/sync-home.bat`、`seed/tool/sqlite.ts`）：`seed/tool/` 一直没有同步步骤（`script/*.bat` 里搜不到 `tool`），两头都断着——引擎侧的原生工具 `~/.redcode/tool/sqlite.ts`（260904 从 MCP 搬过来）不在仓里，重装换机即丢；`seed/tool/` 里那两个 github 工具则永远到不了 `~/.redcode/tool/`。补的这段是**选择性单文件复制**，不整目录镜像：那两个 github 工具是上游 CI 帮手（读 `ISSUE_NUMBER`、import `@redcode-ai/plugin`），实测从 `~/.redcode/tool/` 解析不了这个包名，而 `tool/registry.ts:208` 对扫到的每个文件 `Effect.promise(() => import(...))` **没有逐文件兜底**——一个加载失败会以 defect 连累整张工具表，不只是少一个工具。`sqlite.ts` 同时收进 seed，此前只活在本机。
...
- **服务器内置局域网缺省密码**（`packages/opencode/src/cli/network.ts`）：`redcode web/serve --hostname 0.0.0.0`（或 `--mdns`）在未设置 `REDCODE_SERVER_PASSWORD` 时，不再拒绝启动，而是自动落到内置缺省密码——用户名 `redcode`、密码 `RedCode0429`，手机/平板连局域网 GUI（`/`）或 TUI 网页终端（`/tui`）时输一次即可。要真正私有仍在环境变量里覆盖；显式置空串视为「明确拒绝在此地址监听」，保留原有的拒绝启动行为。回环绑定（默认 127.0.0.1）完全不受影响，本地无鉴权用法与桌面版各自生成的随机密码照旧。同轮清理了 `server.ts` 里 260824 的过时注释——「手机触屏按不出方向键会卡死在选择器」的前提已失效（0.9.7 起 TUI 支持鼠标点选，网页终端实测可点进工作区）。
...
- **OpenAI Chat tool-call 续帧保留已有身份**（`packages/llm/src/protocols/utils/tool-stream.ts`，回归 `test/tool-stream.test.ts` 与 `test/provider/openai-chat.test.ts`）：兼容 provider 在后续 delta 发送空字符串或 `null` 的 `id`/`name` 时，不再覆盖首帧身份；首帧仍要求真实的非空身份。
