...
- **TextNodeRenderable 裸 number 渲染崩溃**：OpenTUI `<text>` 只接受 string，多处直接渲染 number 导致致命错误。涉及底栏 cacheHitPct、侧边栏 messageCount、session-v2 numResults、dialog-status MCP/LSP/formatter/plugin count、footer permissions/lsp length、index reverted length，共 10 处全部改为模板字符串（`prompt/index.tsx`、`sidebar/context.tsx`、`session-v2.tsx`、`dialog-status.tsx`、`footer.tsx`、`index.tsx`）
#### 变更
