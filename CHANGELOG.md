...
- **虚拟列表首个视口尺寸不再等异步回调**（`packages/app/src/pages/session/message-timeline.tsx`）：关闭后台节流后仍能复现首屏空白，用 CDP 读 Virtua 内部状态定位到第二个独立成因——它的 `viewportSize` 初始为 0，而区间计算在拿到首个非零尺寸前返回空区间（内部 `S=false → [0,-1]`），因此整屏无渲染；该尺寸来自 `window resize` 回调与 ResizeObserver，首屏同步渲染会把这次初始化推迟到阻塞结束之后，会话越大、阻塞越久，空白持续越久（CPU 降速 6x 实测该窗口 584ms）。挂载后主动派发一次 `resize` 唤醒，同条件实测缩短到 113ms。
- **遮挡判定下的 rAF 节流不再让虚拟列表卡死**（`packages/app/src/pages/session/message-timeline.tsx`）：关闭后台节流只关掉了 Page Visibility 那一层——Chromium 仍会因窗口被完全遮挡把 rAF 节流到 1Hz（CDP 实测窗口在后台时帧间隔恒定卡在 1000/1007 ms、FPS 1，用 user32 `ShowWindow(9)+SetForegroundWindow` 拉回前台立刻恢复到 FPS 53 / 19 ms），而 Electron 没有为这一层提供 `webPreferences` 开关。`virtua` 的区间初始化与底部锚定都走 rAF，被节流后停在空区间且不自愈，而遮挡场景下 `visibilityState` 恒为 visible，原有的 `visibilitychange` 兜底永远不会触发。补一个窗口 `focus` 监听复用同一次 1px 滚动往返（CDP 验证：派发 `focus` 后确实产生 scroll 事件且 `scrollTop` 回到原值，用户无感）。
