...
- **手机远程访问入口：桌面 bat 与 /lan 命令**（新增 `script/redcode-lan.bat`、`seed/command/lan.md`）：躺床上要用手机连过来时，此前只能自己拼 `redcode web --hostname 0.0.0.0` 再翻环境变量找密码。现在桌面双击 `RedCode-LAN.bat`，或在 TUI 里敲 `/lan`，都会给出手机地址、用户名与当前生效的密码。bat 会先探测仓库根、再 cd 到 `packages/opencode` 后启动——exe 的内嵌 web UI 在运行期未生效，靠该目录下的 `redcode-web-ui.gen.ts` 兜底，换目录启动根路径会 500。`/lan` 的端口状态用内联 shell 注入（命令里刻意不写 `$` 变量：模板经 PowerShell 执行时会被外层先展开成空串）。
