...
- **会话页空白锁定 Chromium 原生窗口遮挡**（`packages/desktop/src/main/index.ts`、`packages/app/src/pages/session/message-timeline.tsx`）：窗口被其他窗口盖住时，Chromium 在 Windows 上的原生遮挡计算（feature `CalculateNativeWinOcclusion`）把页面判为 hidden，于是跳过布局、ResizeObserver 不再回调，`virtua` 的 `viewportSize` 永远停在 0（内部区间门 `S=false → [0,-1]`），可视区间恒为空 → 整片空白，且遮挡解除前不自愈（CDP 实测：`document.hidden=true`、ResizeObserver 回调数为 0、DOM 中唯一的行是 `visibility: hidden` 的占位项，而 `window.innerHeight` 与容器 `clientHeight` 都正常）。此前加的 `backgroundThrottling: false` 只关掉 Page Visibility 那一层——实测打开后 `document.hidden` 仍为 true，Electron 也没有为遮挡判定提供 `webPreferences` 开关——因此在主进程关掉该 feature；一次性对照实验：同一份最小 Electron 脚本，不关时被遮挡窗口 `document.hidden` 恒 `true`，关掉后恒 `false`。渲染侧保留兜底：`visibilitychange` 与窗口 `focus` 时若 `viewportSize` 仍为 0，用 keyed 重建让虚拟列表重新测量并重新 observe。
