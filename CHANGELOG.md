...
- **Bun 1.3.14 → 1.4.0**（`package.json` 的 `packageManager`）：仓里只有这一处钉版本，CI 的 `.github/actions/setup-bun` 是 `node -p "require('./package.json').packageManager.split('@')[1]"` 现取的，改这一行就全覆盖（已核 `bun-v1.4.0` 的 linux/windows/darwin x64 baseline zip 三个产物都在，下载 URL 不会 404）。官方数据里与本仓对得上的几条：**Windows 冷启动 39.0ms → 15.5ms**（编译版 exe 双击那条路径）、**FFI 调用 2.13ns → 0.70ns**（opentui 整条渲染路径就是 Bun FFI）、**JSC 补上与 V8 的 RegExp 差距——`marked.parse()` 80KB markdown 912ms → 6ms**（core 8 个文件 + ui 6 个文件在用 marked，等于不升 marked 白拿）、长驻进程 CPU p99 24% → 10%、`bun install` 首装 15×／CI 冷缓存 19×。本地 bun 二进制不在本次改动范围内，需各自 `bun upgrade`——不升的话跑的还是 1.3.14，只有 CI 会拿到新版。
