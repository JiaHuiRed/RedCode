...
- **编译版 exe 冷启动下默认色文字/宽字符不可见（部分修复，中文仍未解决）**（`cli/cmd/tui/win32.ts`、`cli/cmd/tui/app.tsx`）：`redcode.exe` 从全新进程冷启动时（Explorer 双击、全新终端窗口——不限于编译版，也不需要经过 workspace selector，普通 `redcode.exe <dir>` 在全新窗口里一样复现；已开着的终端里敲命令不会触发），opentui 的终端能力探测经常来不及在超时窗口内完成，`capabilities.unicode`/`rgb`/`explicit_width` 摊在保守默认值上——症状是默认主题色文字、ASCII logo、i18n 快捷键标签、输入框内容不可见，强调色不受影响。根因在闭源原生库内部，改不了；修法：新增 `win32ForceTerminalCapabilities()`，Windows 下默认设置 `OPENTUI_FORCE_WCWIDTH=1`、`OPENTUI_FORCE_EXPLICIT_WIDTH=1`、`COLORTERM=truecolor`（用户已显式设置的不覆盖），绕开探测。三个开关均通过手动复现单独验证有效，修复了静态文字（状态栏、logo 外的提示行）和输入框里的拉丁/ASCII 字符。**已知未解决**：中文（及 logo 的 Unicode 块字符）在输入框里仍不可见，就算三个开关全开也一样——这不再是能力协商问题（能开的都开了），像是原生 `EditBuffer` 宽字符绘制路径本身的 bug，需要看得到源码才能继续查，日常"终端里敲 `redcode` 命令"这条路径未受影响可继续使用。附带给 `cli/project-selector.ts` 的 `cleanup()` 加了 stdin 排空防止选择器退出时残留按键漏给下一个 reader，这是防御性加固，不是本问题的根因（已排除：bug 在跳过 selector 的情况下同样复现）。opentui 升级到 0.4.3 的可行性已调研完成（详见下次记录），机械改动量小，但需专门验证是否解决中文问题。
