...
| 测试纪律"证据面匹配 + 永不默认全量"入规 | 根 AGENTS.md "Run relevant checks locally" | AGENTS.md | 本批 |
| 决策记录制度(notes) | `.agents/notes/`(1369 篇,四态,同 PR 附 note) | `docs/notes/` | 本批 |

## 第二批(小机制,高性价比)

- [ ] **timeout-policy**:工具定义自声明 `timeoutMs`,执行层统一 cooperative 超时(信号通知不硬杀),超时产出结构化 `TOOL_TIMEOUT` 结果供模型自纠。参考 `guard/timeout-policy`。
