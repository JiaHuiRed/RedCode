...
`2026-09-18-background-task-cancellation-and-worktree-cleanup.md` 为隔离 task 增加了成功后的七天
延时回收与失败/取消即时回收；本文件的创建时 `reap()` 保留为 sidecar 重启后未到期延时任务丢失时的
兜底，而不再是成功 task 唯一的回收机会。
