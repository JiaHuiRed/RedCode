...
## 模型可见改动的三问（AGENTS.md 要求必答，补记）

前两次提交只答了第 1 问，第 2、3 问漏了 —— 而项目 AGENTS.md 明写第 3 问"最容易漏、代价最大"。补齐：

**1. 模型看到什么变了。** 21 份模型提示词各自去掉了语气/详略条款、加上一行让位条款；
`beast.md`（gpt-4/o1/o3）与 `copilot-gpt-5.md`（死文件）整份删除，前者连 `system.ts` 路由分支一起。
soul 模板的三节从空占位换成中性默认值。逐条原文对照见上面各节与 CHANGELOG。

**2. token 影响**（`Token.estimate`，即 `prefix-shape.ts` 用的同一个估算器）。
提示词是**单选**的，一个会话只吃其中一份，所以要看单份而不是合计：

| 提示词 | 前 | 后 | 差 |
|---|---|---|---|
| `trinity.md` | 1937 | 1248 | **-689** |
| `deepseek.md` | 2511 | 2173 | **-338** |
| `glm.md` | 1861 | 1666 | -195 |
| `gemini.md` | 3843 | 3656 | -187 |
| `sensenova.md` | 1503 | 1480 | -23 |
| `anthropic.md` | 2049 | 2029 | -20 |
| `default.md` | 1654 | 1646 | -8 |
| `hy.md` | 1740 | 1733 | -7 |
| `kimi` / `mimo` / `qwen` / `minimax` / `ollama` | — | — | **+6 ~ +16** |

最后一组是净增：它们本来就没有语气条款可删，只加了那行让位条款。这是有意的代价——
一行的钱换掉"soul 被 model 档悄悄拽回"的风险。`gpt.md` / `grok.md` / `step.md` 本次未动。
主力两档（deepseek / glm）每轮省 338 / 195 token。

**3. KV cache 影响。** 模型提示词是 `llm/request.ts` 里 `[agentPrompt, ...input.system]` 的
**第 0 段**，动它等于整条前缀作废——**但只作废一次**：升级后每个会话的第一轮重建，之后照常命中。
把让位条款挪到文件开头（如 deepseek 从第 33 行挪到第 9 行）属于段落顺序变更，按规矩会让
"从该段起往后"全部作废；因为第 0 段本来就在动，**没有额外代价**。

关键是没有引入**每轮**的不稳定：改的全是静态文本，不含时间戳、不含按轮变化的内容——
即 `19b2bed3` 和 image resize 文案那两笔账的失效模式，这次不适用。
