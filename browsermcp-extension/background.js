...
async function handleMessage(msg) {
  ...
  switch (type) {
    case 'browser_navigate': return await navigate(payload.url);
    case 'browser_go_back': return await goBack();
    case 'browser_go_forward': return await goForward();
    case 'browser_snapshot': return await getSnapshot();
    case 'browser_click': return await clickElement(payload.element);
    case 'browser_type': return await typeText(payload.element, payload.text);
    case 'browser_hover': return await hoverElement(payload.element);
    case 'browser_select_option': return await selectOption(payload.element, payload.value);
    case 'browser_press_key': return await pressKey(payload.key);
    case 'browser_wait': return await wait(payload.time);
    case 'browser_screenshot': return await screenshot();
    case 'browser_get_console_logs': return await getConsoleLogs();
    case 'getUrl': return await getUrl();
    case 'getTitle': return await getTitle();
...
async function typeText(ref, text) {
  return await execInTab((ref, text) => {
    let el = document.querySelector(`[data-ref="${ref}"]`);
    if (!el) {
...
async function selectOption(ref, value) {
  return await execInTab((ref, value) => {
    let el = document.querySelector(`[data-ref="${ref}"]`);
