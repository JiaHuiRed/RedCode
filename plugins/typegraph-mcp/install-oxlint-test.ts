...
function runTsx(toolRoot: string, args: string[], cwd: string, env: NodeJS.ProcessEnv = process.env): string {
...
function assertIncludes(text: string, expected: string): void {
  assert.ok(text.includes(expected), `Expected output to include:\n${expected}\n\nActual output:\n${text}`)
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const setupOutput = runTsx(repoRoot, [path.join(repoRoot, "cli.ts"), "setup", "--yes"], projectRoot, testEnv)
    ...
    const checkOutput = runTsx(pluginRoot, [path.join(pluginRoot, "cli.ts"), "check"], projectRoot, testEnv)
