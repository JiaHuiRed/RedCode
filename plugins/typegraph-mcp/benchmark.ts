...
import { dependencyTree, dependents, importCycles, shortestPath, subgraph, moduleBoundary } from "./graph-queries.js"
...
function grepCount(pattern: string): { matches: number; files: number; totalBytes: number } {
  try {
    const result = execSync(`grep -r --include='*.ts' --include='*.tsx' -l "${pattern}" . 2>/dev/null || true`, {
      cwd: projectRoot,
      encoding: "utf-8",
    ...
    const countResult = execSync(`grep -r --include='*.ts' --include='*.tsx' -c "${pattern}" . 2>/dev/null || true`, {
      cwd: projectRoot,
      encoding: "utf-8",
      maxBuffer: 10 * 1024 * 1024,
    }).trim()
  ...
  for (const barrel of barrels) {
    ...
    for (const edge of edges) {
      if (
        ...
        const parentBarrels = (graph.reverse.get(barrel) ?? []).filter((e) => path.basename(e.target) === "index.ts")
...
async function benchmarkTokens(client: TsServerClient, graph: ModuleGraph): Promise<TokenScenario[]> {
...
async function benchmarkLatency(client: TsServerClient, graph: ModuleGraph): Promise<LatencyResult[]> {
...
async function benchmarkAccuracy(client: TsServerClient, graph: ModuleGraph): Promise<AccuracyScenario[]> {
