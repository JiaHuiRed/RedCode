...
function findInNavBar(items: NavBarItem[], symbol: string): { line: number; offset: number; kind: string } | null {
...
const exportKinds = new Set(["function", "const", "class", "interface", "type", "enum", "var", "let", "method"])
...
function normalizeExportKindLabel(kind: string, exportKind: ModuleExportRecord["exportKind"]): string {
...
async function getModuleExports(file: string, visited = new Set<string>()): Promise<ModuleExportRecord[]> {
  ...
  for (const exp of parsed.module.staticExports) {
    for (const entry of exp.entries) {
      ...
      const targetFile = resolveProjectImport(moduleResolver, path.dirname(absFile), moduleRequest.value, projectRoot)
      ...
      if (importKind === "AllButDefault" && exportKind === "None") {
        ...
        for (const nested of nestedExports) {
          ...
          const starExportKind: ModuleExportRecord["exportKind"] = entry.isType ? "type" : nested.exportKind
      ...
      const nestedMatch =
        targetFile && importedSymbol
          ? ((await getModuleExports(targetFile, nextVisited)).find((item) => item.symbol === importedSymbol) ?? null)
          : null
      ...
      const resolvedExportKind: ModuleExportRecord["exportKind"] =
        entry.isType || nestedMatch?.exportKind === "type" || kindImpliesTypeOnly(nestedMatch?.kind ?? metadata.kind)
          ? "type"
          : "value"
      const resolvedKind = normalizeExportKindLabel(nestedMatch?.kind ?? metadata.kind, resolvedExportKind)
    ...
    for (const entry of exp.entries) {
      ...
      const exportLoc = offsetToLineColumn(source, entry.exportName.start ?? entry.localName.start ?? entry.start)
      const lookupLoc = offsetToLineColumn(source, exportLookupOffset(entry))
      const metadata = await resolveExportMetadata(
        relFile,
        lookupLoc.line,
  ...
  return [...exportMap.values()].sort((a, b) => a.line - b.line || a.symbol.localeCompare(b.symbol))
}
...
mcpServer.tool(
  ...
  async ({ file }) => {
    ...
    const hasLocalRuntimeExports = exports.some((item) => item.source === "local" && !item.isTypeOnly)
