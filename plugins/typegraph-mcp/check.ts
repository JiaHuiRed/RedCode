...
function hasCompleteJsonTypegraphRegistration(config: unknown, serverName: string, projectRoot: string): boolean {
...
function hasDeclaredDependency(packageJson: Record<string, unknown> | null, packageName: string): boolean {
  const depKeys = ["dependencies", "devDependencies", "peerDependencies", "optionalDependencies"] as const
...
export async function main(configOverride?: TypegraphConfig): Promise<CheckResult> {
      ...
      try {
        ...
        if (tsNav) {
          ...
          if (hasCommand && hasArgs && hasEnv) {
            ...
            fail(`MCP registration incomplete: ${issues.join(", ")}`, "See README for correct .claude/mcp.json format")
          }
        } else {
          const serverPath = toolIsEmbedded ? `./${toolRelPath}/server.ts` : path.resolve(toolDir, "server.ts")
          fail(
  ...
  if (fs.existsSync(toolNodeModules)) {
    ...
    const missing = requiredPkgs.filter((pkg) => !fs.existsSync(path.join(toolNodeModules, ...pkg.split("/"))))
    if (missing.length === 0) {
  ...
  if (tsVersion) {
    try {
      ...
      if (ok) {
        ...
        fail("tsserver did not respond", "Verify `typescript` is installed and tsconfig.json is valid")
      }
    } catch (err) {
      fail(
  ...
  try {
    let buildGraph: (
      root: string,
      tsconfig: string,
    ) => Promise<{ graph: { files: Set<string>; forward: Map<string, unknown[]> } }>
    try {
    ...
    const edgeCount = [...graph.forward.values()].reduce((s: number, e: unknown[]) => s + e.length, 0)
    if (graph.files.size > 0 && edgeCount > 0) {
      ...
      fail("Module graph: 0 files discovered", "Check tsconfig.json includes source files and project root is correct")
    }
  } catch (err) {
    fail(
  ...
  if (fs.existsSync(gitignorePath)) {
    ...
    const ignoresClaude = lines.some((l: string) => l === ".claude/" || l === ".claude" || l === "/.claude")
    ...
    const ignoresParent =
      parentDir && lines.some((l: string) => l === `${parentDir}/` || l === parentDir || l === `/${parentDir}`)
