...
function findExport(result: ModuleExportsResult, symbol: string, exportKind: "value" | "type"): ModuleExportRecord {
  const found = result.exports.find((item) => item.symbol === symbol && item.exportKind === exportKind)
...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert.equal(
      barrel.exports.some((item) => item.symbol === "default"),
      false,
    )
    assert.equal(
      barrel.exports.some((item) => item.symbol === "buildUser"),
      false,
    )
    ...
    assert.equal(
      collision.exports.some((item) => item.symbol === "dup"),
      false,
    )
