...
function findInNavBar(items: NavBarItem[], predicate: (item: NavBarItem) => boolean): NavBarItem | null {
...
function findImporter(graph: ModuleGraph, file: string): string | null {
  ...
  const preferred = revEdges.find((e) => !e.target.includes(".test.") && !e.target.endsWith("index.ts"))
...
export async function main(configOverride?: TypegraphConfig): Promise<SmokeTestResult> {
  ...
  if (failed === 0) {
    console.log(`${passed}/${total} passed` + (skipped > 0 ? ` (${skipped} skipped)` : "") + " -- all tools working")
  } else {
    console.log(
