 ...
 */
export function dependencyTree(graph: ModuleGraph, file: string, opts: DepTreeOpts = {}): DepTreeResult {
 ...
 */
export function dependents(graph: ModuleGraph, file: string, opts: DependentsOpts = {}): DependentsResult {
  const { depth = Infinity, includeTypeOnly = false } = opts
  const visited = new Set<string>()
  const result: string[] = []
  let directCount = 0
 ...
 */
export function importCycles(graph: ModuleGraph, opts: CycleOpts = {}): CycleResult {
 ...
 */
export function shortestPath(graph: ModuleGraph, from: string, to: string, opts: PathOpts = {}): PathResult {
 ...
 */
export function subgraph(graph: ModuleGraph, files: string[], opts: SubgraphOpts = {}): SubgraphResult {
 ...
 */
export function moduleBoundary(graph: ModuleGraph, files: string[]): BoundaryResult {
