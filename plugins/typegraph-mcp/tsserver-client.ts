...
export class TsServerClient {
  ...
  async start(): Promise<void> {
    ...
    try {
      tsserverPath = createRequire(path.resolve(this.projectRoot, "package.json")).resolve("typescript/lib/tsserver.js")
    } catch {
    ...
    if (message.type === "response" && message.request_seq !== undefined) {
      ...
      if (pending) {
        ...
        if (message.success) {
          ...
          pending.reject(new Error(`tsserver ${pending.command} failed: ${message.message ?? "unknown error"}`))
        }
      }
    }
