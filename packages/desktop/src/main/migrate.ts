...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.redcode.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `redcode.settings.dat` is special: it maps to the `redcode.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"redcode.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // redcode.settings.dat → the electron settings store ("redcode.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "redcode.global.dat").
  const storeName = filename === "redcode.settings.dat" ? "redcode.settings" : filename
