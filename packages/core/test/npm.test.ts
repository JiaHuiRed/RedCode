...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@redcode/acme")).toBe("@redcode/acme")
    expect(Npm.sanitize("@redcode/acme@1.0.0")).toBe("@redcode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/redcode/acme.git"
    const expected = win ? "acme@git+https_//github.com/redcode/acme.git" : spec
