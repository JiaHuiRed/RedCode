...
import { DynamicProviderPlugin } from "@redcode-ai/core/plugin/provider/dynamic"
...
function tempEntrypoint(source: string) {
  return Effect.acquireRelease(
    Effect.promise(async () => {
      const directory = await fs.mkdtemp(path.join(os.tmpdir(), "redcode-provider-dynamic-"))
