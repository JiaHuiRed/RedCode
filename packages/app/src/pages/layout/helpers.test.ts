...
describe("layout deep links", () => {
  ...
  test("ignores non-project deep links", () => {
    expect(parseDeepLink("RedCode://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("RedCode://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("RedCode://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("RedCode://open-project")).toBeUndefined()
    expect(parseDeepLink("RedCode://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "RedCode://open-project?directory=/a",
      "RedCode://other?directory=/b",
      "RedCode://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("RedCode://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("RedCode://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("RedCode://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("RedCode://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "RedCode://new-session?directory=/a",
      "RedCode://open-project?directory=/b",
      "RedCode://new-session?directory=/c&prompt=ship%20it",
    ])
