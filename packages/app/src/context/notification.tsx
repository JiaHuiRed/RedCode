...
import { EventSessionError } from "@redcode-ai/sdk/v2"
  ...
  init: () => {
    ...
    const unsub = globalSDK.event.listen((e) => {
      ...
      if (event.type === "message.updated") {
        const info = (event.properties as { info?: { role?: string; parts?: Array<{ type?: string; text?: string }> } })
          .info
        ...
        if (info?.role === "user" && text) {
          ...
          if (settings.notifications.agent()) {
            void platform.notify(
              ok
                ? language.t("notification.session.backgroundComplete.title")
                : language.t("notification.session.backgroundFailed.title"),
              description,
              `/${e.name}`,
            )
          }
