...
export interface ProtocolStream<Frame, Event, State> {
  ...
  /**
   * Initial parser state, built once per response.
   *
   * Takes the request that produced the response so a parser can seed state
   * from options the wire events never repeat back — e.g. OpenAI Responses
   * `store`, which decides when a reasoning block may be closed. Parsers that
   * need nothing from the request simply ignore the parameter.
   */
