---
title: プラグイン
description: RedCode を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで RedCode を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、RedCode のデフォルトの動作を変更したりできます。
...
- `.redcode/plugins/` - プロジェクトレベルのプラグイン
- `~/.redcode/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/redcode/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.redcode/redcode.json`)
2. プロジェクト設定 (`redcode.json`)
3. グローバルプラグインディレクトリ (`~/.redcode/plugins/`)
4. プロジェクトプラグインディレクトリ (`.redcode/plugins/`)
...
RedCode は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための RedCode SDK クライアント。
...
ここでは、RedCode を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://redcode.dev/docs/sdk) を参照してください。
...
RedCode が `.env` ファイルを読み取らないようにします。
...
プラグインは RedCode にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、RedCode が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して RedCode で使用できます。
