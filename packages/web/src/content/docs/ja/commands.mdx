...
`.redcode/commands/test.md` を作成します。
...
カスタムコマンドは、RedCode 設定を通じて、または `commands/` ディレクトリに Markdown ファイルを作成することによって追加できます。

---

### JSON

RedCode [設定](/docs/config)で `command` オプションを使用します:
...
- グローバル: `~/.redcode/commands/`
- プロジェクトごと: `.redcode/commands/`
...
RedCode には、`/init`、`/undo`、`/redo`、`/share`、`/help` などのいくつかの組み込みコマンドが含まれています。 [詳細はこちら](/docs/tui#commands)。
