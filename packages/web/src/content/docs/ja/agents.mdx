...
RedCode には 2 種類のエージェントがあります。プライマリエージェントとサブエージェント。

---

### プライマリエージェント

プライマリエージェントは、直接対話する主要なアシスタントです。 **Tab** キー、または設定した `switch_agent` キーバインドを使用して、それらを切り替えることができます。これらのエージェントが主な会話を処理します。ツールへのアクセスは権限によって構成されます。たとえば、RedMind ではすべてのツールが有効になっていますが、Plan は制限されています。

:::tip
**Tab** キーを使用して、セッション中にプライマリエージェントを切り替えることができます。
:::

RedCode には、**RedMind** と **Plan** という 2 つの組み込みプライマリエージェントが付属しています。これらについて以下で説明します。
...
RedCode には、**Explore** と **Execute** という 2 つの組み込みサブエージェントが付属しています。これについては以下で見ていきます。

---

## 組み込み

RedCode には、2 つの組み込みプライマリエージェントと 2 つの組み込みサブエージェント、さらに 3 つの非表示システムエージェントが付属しています。
...
RedMind は **デフォルト** のプライマリエージェントです。通常の操作（読み取り、書き込み、検索）は
自動で実行し、`bash` などの機微な操作は先に確認を取ります。日常の開発作業の標準エージェントです。
...
計画と分析のために設計された制限付きエージェントです。権限システムにより、意図しない変更を防ぎます。
デフォルトでは、以下のすべてが `ask` に設定されています:

- `file edits`: すべての書き込み、パッチ、編集
- `bash`: すべての bash コマンド

コードベースに実際の変更を加えずに、LLM にコードの分析、変更の提案、計画の作成をさせたいときに有用です。
...
読み取り専用の調査エージェントで、何も変更しません。プロンプトでどの仕事かを伝えてください:

- **探す** — パターンでファイルを見つける、キーワードでコードを検索する、「X はどう動くのか」に答える
- **設計する** — 現状を読んだうえで、計画を出す、技術選定を評価する、アーキテクチャを整理する
- **レビューする** — 変更を読み、バグ・リスク・品質上の問題を報告する
...
読み書き可能な実装エージェントです。方針が固まっていて、実際にコードを変更する・テストを走らせる・
バグを直す必要があるときに使います。計画も調査もしません。明確な作業単位を並列に流すのにも使えます。

---

### 旧称

`build`、`general`、`architect`、`reviewer`、`fixer`、`scout`、`advise` はエイリアス表を経由して
今も解決されます（`build` → `redmind`、`general` と `fixer` → `execute`、それ以外 → `explore`）。
エージェント一覧にも `@` 補完にも現れないため、新しい設定では使わないでください。エイリアスは
**恒久的**です。古いセッションが旧称を再生し続けるため、移行期間後に削除する予定はありません。
...
`redcode.json` の `agent` ブロックは、**すでに存在するエージェントの上書きまたは無効化**のみ
行えます。新規作成はできません（未知のキーは警告付きで無視されます）。新しいエージェントを
追加するには [Markdown ファイル](#markdown)を書いてください。
...
温度が指定されていない場合、RedCode はモデル固有のデフォルトを使用します。通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55 です。
...
このパスは、設定ファイルが配置されている場所に対する相対パスです。したがって、これはグローバルな RedCode 設定とプロジェクト固有の設定の両方で機能します。
...
RedCode 設定内のモデル ID は、`provider/model-id` という形式を使用します。たとえば、[RedCode Zen](/docs/zen) を使用している場合、GPT 5.1 Codex には `redcode/gpt-5.1-codex` を使用します。
...
:::tip
`redcode models` を実行して、利用可能なモデルのリストを表示します。
:::
...
:::tip
共有したいエージェントはいますか? [PR](https://github.com/JiaHuiRed/RedCode) を送信します。
:::
