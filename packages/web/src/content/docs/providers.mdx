---
title: Providers
description: Using any LLM provider in RedCode.
---

import config from "../../../config.mjs"
export const console = config.console

RedCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your RedCode config.
...
in `~/.local/share/redcode/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your RedCode
config.
...
RedCode Zen is a list of models provided by the RedCode team that have been
tested and verified to work well with RedCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with RedCode Zen.
:::

1. Run the `/connect` command in the TUI, select `RedCode Zen`, and head to [redcode.dev/auth](https://redcode.dev/zen).
...
It works like any other provider in RedCode and is completely optional to use.

---

## RedCode Go

RedCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the RedCode team that have been
tested and verified to work well with RedCode.

1. Run the `/connect` command in the TUI, select `RedCode Go`, and head to [redcode.dev/auth](https://redcode.dev/zen).
...
To use Amazon Bedrock with RedCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running redcode:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY redcode

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile redcode

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX redcode
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `redcode.json`:
...
RedCode. Anthropic explicitly prohibits this.

Previous versions of RedCode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in RedCode with zero setup:
...
You can configure redcode to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="redcode.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://redcode.dev/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for redcode to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX redcode
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX redcode
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your redcode config.
...
RedCode supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; RedCode auto-creates a Model Access Key and discovers your available Models & Inference Routers.
...
3. Your browser opens to authorize RedCode. Sign in and approve.

   :::note
   RedCode creates a Model Access Key named `opencode-oauth-<timestamp>` in your DigitalOcean account. You can rotate or revoke it from the **Model Access Keys** page in the "Manage" section of the DigitalOcean console under Inference.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, RedCode surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — RedCode forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in RedCode is experimental. Features, configuration, and
behavior may change in future releases.
:::

RedCode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in redcode auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
RedCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock RedCode
to only use your own GitLab-hosted instance, add the following to your
`redcode.json` file. It is also recommended to disable session sharing.
...
Customize through `redcode.json`:
...
When a `duo-workflow-*` model is selected, RedCode will:
...
4. Route tool execution requests through RedCode's permission-gated tool system
...
To use your GitHub Copilot subscription with redcode:
...
To use Google Vertex AI with RedCode:
...
2. Set the required environment variables:
   ...
   Set them while running redcode.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id redcode
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through redcode, you can always configure it yourself.
...
```jsonc title="~/.redcode/redcode.jsonc"
{
  "$schema": "https://redcode.dev/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each RedCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each RedCode conversation listed as a separate session.
...
You can configure redcode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="redcode.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://redcode.dev/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure redcode to use local models through LM Studio.

```json title="redcode.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://redcode.dev/config.json",
  "provider": {
    "lmstudio": {
...
You can configure redcode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for RedCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::

```json title="redcode.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://redcode.dev/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with RedCode:
...
3. Copy the API key for use in RedCode.
...
6. **Important**: Before using cloud models in RedCode, you must pull the model information locally:
...
RedCode Zen is a list of tested and verified models provided by the RedCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>RedCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **RedCode Zen**.

   ```txt
   /connect
   ```

3. Enter your RedCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your redcode config.
...
5. You can also customize them through your redcode config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' redcode
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group redcode
   ```
...
You can also customize models through your redcode config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. RedCode opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
RedCode refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when RedCode is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running RedCode — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. RedCode prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. RedCode polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with redcode. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in redcode.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `redcode.json` file in your project directory:

   ```json title="redcode.json" ""myprovider"" {5-15}
   {
     "$schema": "https://redcode.dev/config.json",
     "provider": {
       "myprovider": {
...
```json title="redcode.json" {9,11,17-20}
{
  "$schema": "https://redcode.dev/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow RedCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `redcode auth list` to see if the credentials
...
2. For custom providers, check the redcode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your redcode config.
