---
title: Server
description: Interact with redcode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `redcode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an redcode client can use.
...
```bash
redcode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Custom domain name for mDNS service | `redcode.local` |
| `--cors`        | Additional browser origins to allow | `[]`             |

`--cors` can be passed multiple times:

```bash
redcode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `redcode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `redcode serve` and `redcode web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password redcode serve
```
...
When you run `redcode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the redcode server to interact with redcode programmatically.
:::

This architecture lets redcode support multiple clients and allows you to interact with redcode programmatically.

You can run `redcode serve` to start a standalone server. If you have the
redcode TUI running, `redcode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the RedCode [IDE](/docs/ide) plugins.
...
The redcode server exposes the following APIs.
