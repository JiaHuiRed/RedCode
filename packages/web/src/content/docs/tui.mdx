---
title: TUI
description: Using the RedCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

RedCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running RedCode starts the TUI for the current directory.
...
When using the RedCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to RedCode. Allows you to select from available providers and add their API keys.
...
This is separate from `redcode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long RedCode waits after the leader key. Defaults to `2000`.
...
- `bell` - On Windows, emit a BEL character (flash the taskbar icon when the terminal is unfocused) together with the notification. Pair with Windows Terminal `bellStyle: "taskbar"` for a WhatsApp-style taskbar flash. Defaults to `true`.
- `volume` - Default sound volume from `0` to `1`. Defaults to `0.4`.
- `sound_pack` - Sound pack ID to use. Defaults to `redcode.default`.
