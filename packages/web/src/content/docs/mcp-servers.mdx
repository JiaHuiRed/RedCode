...
You can add external tools to RedCode using the _Model Context Protocol_, or MCP. RedCode supports both local and remote servers.
...
You can define MCP servers in your [RedCode Config](https://redcode.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="redcode.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/redcode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
RedCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, RedCode will:
...
If the server requires authentication, RedCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `redcode mcp auth <server-name>`.
...
```bash
redcode mcp auth my-oauth-server
```
...
```bash
redcode mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, RedCode will store the tokens securely in `~/.local/share/redcode/mcp-auth.json`.
...
redcode mcp debug my-oauth-server
```
...
Your MCPs are available as tools in RedCode, alongside built-in tools. So you can manage them through the RedCode config like any other tool.
...
```json title="redcode.json" {11, 14-18}
{
  "$schema": "https://redcode.dev/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
redcode mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect RedCode to your Sentry account.
