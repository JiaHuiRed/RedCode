---
title: CLI
description: RedCode CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The RedCode CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
redcode
```

But it also accepts commands as documented on this page. This allows you to interact with RedCode programmatically.

```bash
redcode run "Explain how closures work in JavaScript"
```
...
Start the RedCode terminal user interface.
...
The RedCode CLI also has the following commands.

---

### agent

Manage agents for RedCode.

```bash
redcode agent [command]
```
...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.redcode/agent` based on the prompt)                                                                                                                         |
...
Attach a terminal to an already running RedCode backend server started via `serve` or `web` commands.

```bash
redcode attach [url]
```

This allows using the TUI with a remote RedCode backend. For example:

```bash
# Start the backend server for web/mobile access
redcode web --port 4096 --hostname 0.0.0.0

# In another terminal, attach the TUI to the running backend
redcode attach http://10.20.30.40:4096
```
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `redcode`) |
...
RedCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `redcode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/redcode/auth.json`.

```bash
redcode auth login
```

When RedCode starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in RedCode.

```bash
redcode models --refresh
```
...
Run redcode in non-interactive mode by passing a prompt directly.
...
```bash "redcode run"
redcode run Explain the use of context in Go
```

You can also attach to a running `redcode serve` instance to avoid MCP server cold boot times on every run:
...
redcode run --attach http://localhost:4096 "Explain async/await in JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>                       |       | Attach to a running redcode server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr>                     | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr>                     | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `redcode`) |
...
Start a headless RedCode server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
redcode serve
```

This starts an HTTP server that provides API access to redcode functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `redcode`).
...
Manage RedCode sessions.

```bash
redcode session [command]
```
...
List all RedCode sessions.

```bash
redcode session list
```
...
Delete an RedCode session.

```bash
redcode session delete <sessionID>
```
...
Show token usage and cost statistics for your RedCode sessions.
...
Import session data from a JSON file or RedCode share URL.

```bash
redcode import <file>
```

You can import from a local file or an RedCode share URL.

```bash
redcode import session.json
redcode import https://opncd.ai/s/abc123
```
...
Start a headless RedCode server with a web interface.

```bash
redcode web
```

This starts an HTTP server and opens a web browser to access RedCode through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `redcode`).
...
Fetch and checkout a GitHub PR branch, then run RedCode.
...
Uninstall RedCode and remove all related files.
...
Updates redcode to the latest version or a specific version.

```bash
redcode upgrade [target]
```
...
The redcode CLI takes the following global flags.
...
RedCode can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `redcode`) |
