...
There are two types of agents in RedCode; primary agents and subagents.

---

### Primary agents

Primary agents are the main assistants you interact with directly. You can cycle through them using the **Tab** key, or your configured `switch_agent` keybind. These agents handle your main conversation. Tool access is configured via permissions — for example, RedMind has all tools enabled while Plan is restricted.

:::tip
You can use the **Tab** key to switch between primary agents during a session.
:::

RedCode comes with two built-in primary agents, **RedMind** and **Plan**. We'll
look at these below.
...
RedCode comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

RedCode comes with two built-in primary agents and two built-in subagents, plus
three hidden system agents.
...
RedMind is the **default** primary agent. Everyday operations (read, write,
search) run automatically; sensitive ones such as `bash` ask first. This is the
...
A read-only investigator. It never modifies anything. Say in the prompt which of
the three jobs you want:

- **find** — locate files by pattern, search code for keywords, or answer "how
  does X work?" about the codebase
- **design** — read the current state, then produce a plan, evaluate a technical
  choice, or map an architecture
- **review** — read a change and report bugs, risks, and quality problems
...
A read-write agent for implementation. Use it once the approach is settled and
you need code changed, tests run, or a bug fixed. It does not plan or research.
Also use it to run several well-defined units of work in parallel.
...
`build`, `general`, `architect`, `reviewer`, `fixer`, `scout`, and `advise` still
resolve through an alias table — `build` to `redmind`, `general` and `fixer` to
`execute`, and the rest to `explore`. They do not appear in any agent list or in
`@` completion, and should not be used in new configuration. The aliases are
permanent: older sessions keep replaying the old names, so they are not going
away after a transition period.
...
The `agent` block in `redcode.json` **overrides or disables an agent that already
exists**. It does not create one — an unknown key there is ignored with a
warning. To add a new agent, write a [markdown file](#markdown).
...
- Global: `~/.redcode/agent/`
- Per-project: `.redcode/agent/`

```markdown title="~/.redcode/agent/review.md"
---
...
If no temperature is specified, RedCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global RedCode config and the project specific config.
...
The model ID in your RedCode config uses the format `provider/model-id`. For example, if you're using [RedCode Zen](/docs/zen), you would use `redcode/gpt-5.1-codex` for GPT 5.1 Codex.
...
```json title="redcode.json" {3-6,9-12}
{
  "$schema": "https://redcode.dev/config.json",
  "tools": {
    "write": true,
...
```json title="redcode.json" {3-5,8-10}
{
  "$schema": "https://redcode.dev/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Run `redcode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/JiaHuiRed/RedCode).
:::
...
```markdown title="~/.redcode/agent/docs-writer.md"
---
...
```markdown title="~/.redcode/agent/security-auditor.md"
---
