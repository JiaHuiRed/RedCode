...
RedCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
RedCode comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into RedCode's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `redcode.json` config file:
...
- Global: `~/.redcode/agents/`
- Per-project: `.redcode/agents/`

```markdown title="~/.redcode/agents/review.md"
---
...
```markdown title="~/.redcode/agents/docs-writer.md"
---
...
```markdown title="~/.redcode/agents/security-auditor.md"
---
