...
To debug issues with RedCode, start by checking the logs and local data it stores on disk.
...
- **macOS/Linux**: `~/.local/share/redcode/log/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\redcode\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `redcode --log-level DEBUG`.

---

## Storage

redcode stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/redcode/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\redcode`
...
RedCode Desktop runs a local RedCode server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `RedCode` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.redcode/redcode.jsonc` (or `~/.redcode/redcode.json`)
- **macOS/Linux** (older installs): `~/.local/share/redcode/redcode.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\redcode\redcode.jsonc`
...
RedCode can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.redcode/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\redcode\plugins`
- **Project plugins** (only if you use per-project config)
  - `<your-project>/.redcode/plugins/`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so RedCode can rebuild it.

1. Quit RedCode Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/redcode`
- **Linux**: delete `~/.cache/redcode` (or run `rm -rf ~/.cache/redcode`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\redcode`

3. Restart RedCode Desktop.

---

### Fix server connection issues

RedCode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `redcode.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, RedCode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with RedCode's features.

---

### Notifications not showing

RedCode Desktop only shows system notifications when:

- notifications are enabled for RedCode in your OS settings, and
...
1. Quit RedCode Desktop.
2. Find and delete these files (they live in the RedCode Desktop app data directory):

- `redcode.settings.dat` (desktop default server URL)
- `redcode.global.dat` and `redcode.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with RedCode:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/JiaHuiRed/RedCode/issues**](https://github.com/JiaHuiRed/RedCode/issues)
...
2. **Join our Discord**
   ...
   [**redcode.dev/discord**](https://redcode.dev/discord)
...
3. Ensure you have the latest version with `redcode upgrade`
...
To figure out what models you have access to, run `redcode models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   ```bash
   rm -rf ~/.local/share/redcode
   ```

   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\redcode`
...
If you encounter API call errors, this may be due to outdated provider packages. redcode dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\redcode`

2. Restart redcode to reinstall the latest provider packages

This will force redcode to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
redcode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
