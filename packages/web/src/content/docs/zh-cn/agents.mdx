...
RedCode 中有两种类型的代理：主代理和子代理。

---

### 主代理

主代理是您直接交互的主要助手。您可以使用 **Tab** 键或配置的 `switch_agent` 快捷键来循环切换它们。这些代理处理您的主要对话。工具访问通过权限进行配置——例如，RedMind 启用了所有工具，而 Plan 则受到限制。

:::tip
您可以在会话期间使用 **Tab** 键在主代理之间切换。
:::

RedCode 内置了两个主代理：**RedMind** 和 **Plan**。我们将在下面介绍它们。
...
RedCode 内置了两个子代理：**Explore** 和 **Execute**。我们将在下面介绍它们。

---

## 内置代理

RedCode 内置了两个主代理和两个子代理，另有三个隐藏的系统代理。
...
RedMind 是**默认**主代理。常规操作（读、写、搜索）自动执行，bash 等敏感操作会先征得同意。
这是日常开发工作的标准代理。
...
只读的调查型代理，绝不修改任何文件。在提示词里说明你要哪一种：

- **找东西** —— 按模式查找文件、搜索关键字，或回答「这块是怎么跑的」
- **出方案** —— 先读清现状，再给方案、评估技术选型或梳理架构
- **做审查** —— 读一份改动，报出 bug、风险与质量问题
...
可读写的实现型代理。当方案已定、需要实际改代码、跑测试、修 bug 时使用；不规划不研究。
也用于把多个明确的工作单元并行铺开。
...
`build`、`general`、`architect`、`reviewer`、`fixer`、`scout`、`advise` 仍可经别名表解析
—— `build` → `redmind`，`general` 与 `fixer` → `execute`，其余 → `explore`。它们不出现在任何
代理列表与 `@` 补全里，新配置不应再用。别名是**长期保留**的：老会话会不断重放旧名字，
不存在「过渡一轮后删掉」。
...
`redcode.json` 里的 `agent` 块只能**覆写或禁用一个已经存在的代理**，不能创建 ——
未知的 key 会被忽略并打一条 warning。要新增代理，写一份 [Markdown 文件](#markdown)。
...
如果未指定温度，RedCode 将使用模型特定的默认值；大多数模型通常为 0，Qwen 模型为 0.55。
...
此路径相对于配置文件所在位置。因此它同时适用于全局 RedCode 配置和项目级配置。
...
RedCode 配置中的模型 ID 使用 `provider/model-id` 格式。例如，如果您使用 [RedCode Zen](/docs/zen)，则可以使用 `redcode/gpt-5.1-codex` 来表示 GPT 5.1 Codex。
...
:::tip
您有想要分享的代理吗？[提交 PR](https://github.com/JiaHuiRed/RedCode)。
:::
