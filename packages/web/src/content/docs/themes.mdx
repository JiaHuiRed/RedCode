...
With RedCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, RedCode uses our own `redcode` theme.
...
RedCode comes with several built-in themes.
...
- Want RedCode to match their terminal's appearance
...
RedCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.redcode/themes/*.json` or `$XDG_CONFIG_HOME/redcode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.redcode/themes/*.json`
4. **Current working directory** - Defined in `./.redcode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.redcode/themes
vim ~/.redcode/themes/my-theme.json
```
...
{
  "$schema": "https://redcode.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
