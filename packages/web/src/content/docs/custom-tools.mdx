---
title: Custom Tools
description: Create tools the LLM can call in redcode.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside redcode's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.redcode/tools/` directory of your project.
- Or globally, by placing them in `~/.redcode/tools/`.
...
```ts title=".redcode/tools/database.ts" {1}
...
```ts title=".redcode/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".redcode/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".redcode/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".redcode/tools/add.py"
import sys
...
```ts title=".redcode/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".redcode/tools/add.py")
