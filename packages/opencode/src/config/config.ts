...
import { NonNegativeInt, PositiveInt, type DeepMutable } from "@redcode-ai/core/schema"
...
function normalizeLoadedConfig(data: unknown, source: string) {
  ...
  delete copy.tui
  log.warn("tui keys in redcode config are deprecated; move them to tui.json", { path: source })
  return copy
}
...
export const Info = Schema.Struct({
  ...
  command: Schema.optional(Schema.Record(Schema.String, ConfigCommand.Info)).annotate({
    description: "Command configuration, see https://redcode.dev/docs/commands",
  }),
  ...
  ).annotate({ description: "Agent configuration, see https://redcode.dev/docs/agents" }),
...
// Uses the shared `DeepMutable` from `@redcode-ai/core/schema`. See the definition
...
function globalConfigFile() {
  const candidates = ["redcode.jsonc", "redcode.json", "config.json"].map((file) => path.join(Global.Path.config, file))
...
function writableGlobal(info: Info) {
  ...
  const patched: Record<string, ConfigAgent.Info | undefined> = {}
  for (const [name, value] of Object.entries(agent)) patched[name] = writableAgent(value)
  // agent 段的 index signature 值类型不含 undefined，但 patchJsonc 正是靠 undefined 删键；
  // REST 请求体传不了 undefined，这是落盘前唯一的删法，所以这里断言。
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.REDCODE_CONFIG && !Flag.REDCODE_CONFIG_DIR && !Flag.REDCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://redcode.dev/config.json" }, null, 2))
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/redcode`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://redcode.dev/config.json"
        ...
        if (Flag.REDCODE_CONFIG) {
          ...
          log.debug("loaded custom config", { path: Flag.REDCODE_CONFIG })
        }
        ...
        if (Flag.REDCODE_CONFIG_DIR) {
          log.debug("loading config from REDCODE_CONFIG_DIR", { path: Flag.REDCODE_CONFIG_DIR })
        }
        ...
        if (process.env.REDCODE_CONFIG_CONTENT) {
          ...
          log.debug("loaded custom config from REDCODE_CONFIG_CONTENT")
        }
        ...
        if (Flag.REDCODE_PERMISSION) {
          try {
            ...
            log.warn("REDCODE_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
