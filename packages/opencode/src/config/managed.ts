...
const MANAGED_PLIST_DOMAIN = "ai.redcode.managed"
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/redcode"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "redcode")
    default:
      return "/etc/redcode"
  }
}
...
export function managedConfigDir() {
  return process.env.REDCODE_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
