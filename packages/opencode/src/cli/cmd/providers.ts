...
import type { Hooks } from "@redcode-ai/plugin"
...
export const ProvidersLoginCommand = effectCmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "redcode auth provider",
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.url) {
      ...
      const wellknown = (yield* cliTry(`Failed to load auth provider metadata from ${url}: `, () =>
        fetch(`${url}/.well-known/redcode`).then((x) => x.json()),
      )) as {
    ...
    if (provider === "redcode") {
      yield* Prompt.log.info("Create an api key at https://redcode.dev/auth")
    }
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://redcode.dev/docs/providers/#cloudflare-ai-gateway",
      )
    }
