...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "redcode.agent.name": input.agent,
      "redcode.directory": input.directory,
      "redcode.first": input.first,
      "redcode.model.provider": input.model?.providerID,
      "redcode.model.id": input.model?.modelID,
      "redcode.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "redcode.show_exit": next.showExit,
