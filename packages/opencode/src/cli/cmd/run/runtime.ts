...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "redcode.mode": input.resolveSession ? "local" : "attach",
      "redcode.initial_input": !!input.initialInput,
      "redcode.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "redcode.directory": ctx.directory,
        "redcode.resume": ctx.resume === true,
        "redcode.agent.name": state.agent,
        "redcode.model.provider": state.model?.providerID,
        "redcode.model.id": state.model?.modelID,
        "redcode.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "redcode.model.provider": model.providerID,
            "redcode.model.id": model.modelID,
            "redcode.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "redcode.model.id": state.model?.modelID,
                "redcode.model.variant": state.activeVariant,
                "redcode.prompt.chars": prompt.text.length,
                "redcode.prompt.parts": prompt.parts.length,
                "redcode.prompt.include_files": includeFiles,
                "redcode.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "redcode.directory": input.directory,
      "redcode.initial_input": !!input.initialInput,
      "redcode.demo": input.demo,
    },
    async () => {
      const sdk = createOpencodeClient({
        baseUrl: "http://redcode.internal",
        fetch: input.fetch,
