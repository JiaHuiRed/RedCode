...
const AGENT_USERNAME = "redcode-agent[bot]"
    ...
    yield* Effect.promise(async () => {
      {
        ...
        function printNextSteps() {
          ...
          prompts.outro(
            [
              ...
              "",
              "   Learn more about the GitHub agent - https://redcode.dev/docs/github/#usage-examples",
            ].join("\n"),
          )
        }
        ...
        async function installGitHubApp() {
          ...
          // Open browser
          const url = "https://github.com/apps/redcode-agent"
          const command =
          ...
          async function getInstallation() {
            return await fetch(
              `https://api.redcode.dev/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
            )
              .then((res) => res.json())
...
jobs:
  redcode:
    if: |
      ...
      startsWith(github.event.comment.body, '/redcode')
    ...
    steps:
      ...
      - name: Run redcode
        uses: JiaHuiRed/opencode/github@latest${envStr}
        with:
...
export const GithubRunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.redcode.dev" : "https://redcode.dev"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.redcode.dev"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/redcode,/oc")
          .split(",")
        ...
        return runLocalEffect(
          Effect.gen(function* () {
            ...
            const result = yield* prompt.prompt({
              ...
              // agent is omitted - server will use default_agent from config or fall back to "redmind"
              parts: [
                {
                  id: PartID.ascending(),
                  type: "text",
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("redcode-github-action")
        } catch (error) {
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/redcode-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[redcode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the redcode infrastructure after your response",
