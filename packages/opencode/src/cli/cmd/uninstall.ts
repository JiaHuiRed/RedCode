...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g redcode-ai",
      pnpm: "pnpm uninstall -g redcode-ai",
      bun: "bun remove -g redcode-ai",
      yarn: "yarn global remove redcode-ai",
      brew: "brew uninstall redcode",
      choco: "choco uninstall redcode",
      scoop: "scoop uninstall redcode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "redcode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# redcode") || content.includes(".opencode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".redcode"))
    ) {
      continue
    }
