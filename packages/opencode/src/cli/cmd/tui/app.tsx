...
function rendererConfig(_config: TuiConfig.Resolved): CliRendererConfig {
  const mouseEnabled = !Flag.REDCODE_DISABLE_MOUSE && (_config.mouse ?? true)
...
export function tui(input: {
  ...
  return new Promise<void>(async (resolve) => {
    ...
    // 260803 Red startup: don't block first render on the theme probe. The native
    // DA/OSC query chain runs on its own; late results reach ThemeProvider via
    // THEME_MODE events (and its init reads renderer.themeMode synchronously).
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        hidden: true,
        run: () => {
          local.model.cycle收藏(1)
        },
      },
      {
        name: "model.cycle_favorite_reverse",
        title: "收藏 cycle reverse",
        category: "Agent",
        hidden: true,
        run: () => {
          local.model.cycle收藏(-1)
      ...
      },
      {
        ...
        title: "打开文档",
        run: () => {
          open("https://redcode.dev/docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  return (
    <box
      ...
      onMouseUp={Flag.REDCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? undefined : () => Selection.copy(renderer, toast)}
    >
