...
const GO_UPSELL_PROVIDERS = new Set(["redcode", "redcode-go"])
...
export function Session() {
  ...
  const thinking = use思考中Mode()
  const thinkingMode = thinking.mode
  const show思考中 = createMemo(() => true)
  ...
  createEffect(() => {
    ...
    return exit.message.set(
      [
        ...
        `  ${weak("Continue")}${UI.Style.TEXT_NORMAL_BOLD}redcode -s ${session()?.id}${UI.Style.TEXT_NORMAL}`,
        ``,
      ].join("\n"),
    )
  })
  ...
  const sessionCommandList = createMemo(() => [
    ...
    },
    {
      ...
      run: () => {
        ...
        if (!selectedModel) {
          toast.show({
            variant: "warning",
            message: "连接 Provider to summarize this session",
            duration: 3000,
          })
          return
        }
    ...
    },
    {
      ...
      run: async () => {
        await sdk.client.session
          ...
          .catch((error) => {
            toast.show({
              message: error instanceof Error ? error.message : "取消分享失败",
              variant: "error",
            })
          })
        dialog.clear()
    ...
    {
      title: (() => {
        const next = next思考中Mode(thinkingMode())
      ...
      run: () => {
        thinking.set(next思考中Mode(thinkingMode()))
        dialog.clear()
      },
    },
    {
...
function ReasoningPart(props: { last: boolean; part: ReasoningPart; message: AssistantMessage }) {
  ...
  // OpenAI / Copilot / redcode-via-OpenAI emit `**Title**\n\n<body>` summary
        ...
        <Match when={!inMinimal() || expanded()}>
          ...
          <box id={"text-" + props.part.id} paddingLeft={3} marginTop={1} flexDirection="column" onMouseUp={toggle}>
            <code
              ...
              streaming={true}
              syntaxStyle={syntax()}
              // `_思考中:_`/`_已思考:_` still drives markdown emphasis color and conceals the underscores;
              // the syntax override above removes only the italic attribute from that emphasis token.
              content={(inMinimal() ? "- " : "") + (isDone() ? "_已思考:_ " : "_思考中:_ ") + content()}
              conceal={ctx.conceal()}
              fg={theme.textMuted}
            />
          </box>
