...
const TUI_SCHEMA_URL = "https://redcode.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from redcode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const redcode = await redcodeFiles(input)
  for (const file of redcode) {
...
async function redcodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "redcode"),
    ...(await Filesystem.findUp(["redcode.json", "redcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "redcode"))
  }
