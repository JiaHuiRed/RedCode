  ...
  private async processMessage(message: SessionMessageResponse) {
    ...
    for (const part of message.parts) {
      if (part.type === "tool") {
        ...
        // RedCode stores files internally as { type: "file", url, filename, mime }.
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const redcodeProvider = providers.find((p) => p.id === "redcode")
  if (redcodeProvider) {
    const [best] = Provider.sort(Object.values(redcodeProvider.models))
    if (best) {
      return {
