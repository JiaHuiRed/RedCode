...
export class Service extends Context.Service<Service, Interface>()("@redcode/Reference") {}
...
function materializeAll(input: { flags: RuntimeFlags.Info; materializers: Materializer[] }) {
  if (!input.flags.experimentalReference) return Effect.void
  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      init: Effect.fn("Reference.init")(function* () {
        if (!flags.experimentalReference) return
      ...
      contains: Effect.fn("Reference.contains")(function* (target?: string) {
        if (!flags.experimentalReference) return false
        const full = normalizedTarget(target)
