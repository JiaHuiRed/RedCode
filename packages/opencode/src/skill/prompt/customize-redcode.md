...
`"$schema": "https://redcode.dev/config.json"` so the user's editor catches
...
- `OPENCODE_CONFIG_CONTENT='{"$schema":"https://redcode.dev/config.json"}'`:
...
- Validate against the schema before writing. If you are unsure of a field's
  ...
  `https://redcode.dev/config.json` and read the schema rather than guessing.
