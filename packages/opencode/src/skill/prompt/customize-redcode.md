<!--
  ...
  packages/redcode/src/skill/index.ts (see CUSTOMIZE_OPENCODE_SKILL_NAME
...
redcode validates its own config strictly and refuses to start when a field
...
**<https://redcode.ai/config.json>**

If a field is not documented in this skill, or you need to confirm an exact
shape before writing config, **fetch that URL and read the schema directly**
rather than guessing. redcode hard-fails on invalid config, so the cost of a
wrong shape is a broken startup.

Independently, every `redcode.json` should declare
`"$schema": "https://redcode.ai/config.json"` so the user's editor catches
mistakes as they type.

## Applying changes

Config is loaded once when redcode starts and is not hot-reloaded. After
saving changes to `redcode.json`, an agent file, a skill, a plugin, or any
other config-time file, **tell the user to quit and restart redcode** for
...
top-level keys in `redcode.json` are rejected with `ConfigInvalidError`.
...
```json
{
  "$schema": "https://redcode.ai/config.json",
  "username": "string",
  ...
  "skills": {
    "paths": [".redcode/skills", "/abs/path/to/skills"],
...
redcode's skill loader scans for `**/SKILL.md` inside skill directories. The
file is named `SKILL.md` exactly, and lives in its own folder named after the
skill:

```
.redcode/skills/my-skill/SKILL.md
```
...
```json
"plugin": [
  "redcode-gemini-auth",            // npm spec, latest
  "redcode-foo@1.2.3",              // npm spec, pinned
  "./local-plugin.ts",               // file path, relative to the declaring config
  "file:///abs/path/plugin.js",      // file URL
  ["redcode-bar", { "key": "val" }] // tuple form with options
]
```

Auto-discovered plugins (no config entry needed): any `*.ts` or `*.js` file in
`.redcode/plugin/` or `.redcode/plugins/`.
...
```ts
import type { Plugin } from "@redcode-ai/plugin"
...
redcode evaluates the LAST matching rule, so put broad rules first and narrow
rules last.
...
- Validate against the schema before writing. If you are unsure of a field's
  ...
  `https://redcode.ai/config.json` and read the schema rather than guessing.
- Preserve `$schema` and any existing fields the user did not ask to change.
- For agent, skill, and plugin definitions, prefer creating new files in the
  correct location over inlining everything in `redcode.json`.
- If the user's existing config is malformed, point them at the env-var escape
  hatches above so they can edit from inside redcode without breaking their
  session.
- After saving any config change, remind the user to quit and restart redcode
