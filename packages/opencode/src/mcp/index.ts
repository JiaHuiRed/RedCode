...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const connectRemote = Effect.fn("MCP.connectRemote")(function* (
      ...
      // 260603 Red P2: 记录 transport 降级候选
      log.info("MCP transport candidates", { key, transports: transports.map((t) => t.name) })
    ...
    const connectLocal = Effect.fn("MCP.connectLocal")(function* (
      ...
      // 260603 Red P2: 记录选择的 transport
      log.info("MCP transport: stdio", { key, command: cmd })
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("MCP.state")(function* () {
        ...
        // 260603 Red P0: MCP 健康监控 — 30s 检查一次，连续 3 次失败标记断开
      ...
      yield* Effect.forEach(
        ...
        ([clientName, client]) =>
          Effect.gen(function* () {
            ...
            // 260603 Red P1: 传入重连函数供工具调用失败时使用
            const doReconnect = async () => {
              await AppRuntime.runPromise(reconnectServer(clientName))
            }
