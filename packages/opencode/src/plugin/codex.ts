...
<html>
  <head>
    <title>RedCode - Codex Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
...
<html>
  <head>
    <title>RedCode - Codex Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
      ...
      async models(provider, ctx) {
        ...
        return Object.fromEntries(
          Object.entries(provider.models)
            .filter(([, model]) => ALLOWED_MODELS.has(model.api.id))
            .map(([modelID, model]) => [
              modelID,
              {
                ...model,
                // 订阅认证按套餐额度计费，不按 token 收钱，所以单价归零。
                cost: {
                  input: 0,
                  output: 0,
