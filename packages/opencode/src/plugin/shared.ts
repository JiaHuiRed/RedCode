...
export const DEPRECATED_PLUGIN_PACKAGES = ["redcode-openai-codex-auth", "redcode-copilot-auth"]
...
export function isPathPluginSpec(spec: string) {
  return spec.startsWith("file://") || spec.startsWith(".") || spec.startsWith("~/") || spec.startsWith("~\\") || isAbsolutePath(spec)
}

export async function resolvePathPluginTarget(spec: string) {
  let raw = spec.startsWith("file://") ? fileURLToPath(spec) : spec
...
export async function checkPluginCompatibility(target: string, redcodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(redcodeVersion) || semver.major(redcodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.redcode
  if (typeof range !== "string") return
  if (!semver.satisfies(redcodeVersion, range)) {
    throw new Error(`Plugin requires redcode ${range} but running ${redcodeVersion}`)
  }
}
