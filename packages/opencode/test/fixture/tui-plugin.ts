...
type Opts = {
  ...
  state?: {
    ...
    provider_quota?: HostPluginApi["state"]["provider_quota"]
  }
  theme?: {
    selected?: string
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  let selected = opts.theme?.selected ?? "redcode"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "redcode.default",
    ...
    state: {
      ...
      lsp: opts.state?.lsp ?? (() => []),
      mcp: opts.state?.mcp ?? (() => []),
      provider_quota: opts.state?.provider_quota ?? (() => []),
    },
    theme: {
      get current() {
