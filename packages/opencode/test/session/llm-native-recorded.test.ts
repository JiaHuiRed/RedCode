...
import { LLMEvent, LLMResponse } from "@redcode-ai/llm"
...
const zenURL = (connection: string) => `https://console.redcode.dev/proxy/connections/${connection}/v1`
...
const RECORDED_SCENARIOS = [
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["redcode", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["redcode", "zen", "native", "tool-loop"],
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the redcode LLM stack remain real.
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://redcode.dev/config.json", ...scenario.config(model) }),
    ),
  )
