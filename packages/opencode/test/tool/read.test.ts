...
describe("tool.read external_directory permission", () => {
  ...
  scout.live("does not ask for external_directory permission when reading configured references", () =>
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const cache = path.join(Global.Path.repos, "github.com", "redcode-read-reference", "repo")
      ...
      const remoteDir = path.join(remoteRoot, "redcode-read-reference")
      ...
      const dir = yield* tmpdirScoped({
        ...
        config: {
          reference: {
            docs: "redcode-read-reference/repo",
          },
        },
      })
...
describe("read.折叠重读", () => {
  ...
  test("拒绝反解被截断的输出", () => {
    const capped = render(body([])).replace(
      "(End of file - total 30 lines)",
      "(Output capped at 50 KB. Showing lines 1-30.)",
    )
