...
import type { TuiAttentionSoundPack } from "@redcode-ai/plugin/tui"
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".redcode", "themes", tmp.extra.globalThemeFile))
      .then(() => true)
      .catch(() => false)
...
test("does not bootstrap server plugins while initializing tui plugins", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(path.join(dir, "redcode.json"), JSON.stringify({ plugin: [pathToFileURL(plugin).href] }))
      return { marker }
    },
  })
