...
// terminal sizes. The default redcode tui command is excluded —
// `redcode --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `redcode --help` prints today;
...
describe("redcode CLI help-text snapshots", () => {
    ...
    ({ redcode }) =>
      Effect.gen(function* () {
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* redcode.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`redcode ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`redcode ${argv.join(" ")} --help`)
        }
