...
describe("redcode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, redcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* redcode.run("say hi")
        redcode.expectExit(result, 0)
  ...
  // (the prompt was accepted, then the upstream provider failed), redcode
  ...
  cliIt.concurrent(
    ...
    ({ llm, redcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* redcode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, redcode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* redcode.run("say hi", { format: "json" })
        redcode.expectExit(result, 0)

        const events = redcode.parseJsonEvents(result.stdout)
