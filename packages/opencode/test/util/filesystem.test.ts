...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "redcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "redcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "redcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "redcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["redcode.json", "redcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "redcode.json"),
        path.join(project, "redcode.jsonc"),
      ])
    })
  })
