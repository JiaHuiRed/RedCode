...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-redcode-directory", directory)
    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* request(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://redcode.dev/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-redcode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-redcode-workspace")
      } finally {
