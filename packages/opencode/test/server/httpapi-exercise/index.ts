...
void (await import("@redcode-ai/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-redcode-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-redcode-directory")
  }),
  ...
  http.protected.get("/provider/quota", "provider.quota").json(200, array),
  http.protected
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "redcode", model: "test" })}`,
      headers: ctx.headers(),
    }))
