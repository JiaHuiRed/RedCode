...
const noAuthLayer = ServerAuth.Config.layer({ password: Option.none(), username: "redcode" })
const secretLayer = ServerAuth.Config.layer({ password: Option.some("secret"), username: "redcode" })
const kitSecretLayer = ServerAuth.Config.layer({ password: Option.some("secret"), username: "kit" })
const colonSecretLayer = ServerAuth.Config.layer({
  password: Option.some("secret:with:colons:"),
  username: "redcode",
})
...
describe("HttpApi authorization middleware", () => {
  itColonSecret.live("accepts colons in passwords", () =>
    Effect.gen(function* () {
      const [header, query, truncated] = yield* Effect.all(
        [
          getProbe({ authorization: basic("redcode", "secret:with:colons:") }),
          HttpClient.get(`/probe?auth_token=${encodeURIComponent(token("redcode", "secret:with:colons:"))}`),
          getProbe({ authorization: basic("redcode", "secret:with:colons") }),
        ],
  ...
  itSecret.live("requires configured password for basic auth", () =>
    Effect.gen(function* () {
      const [missing, badPassword, good] = yield* Effect.all(
        [
          getProbe(),
          getProbe({ authorization: basic("redcode", "wrong") }),
          getProbe({ authorization: basic("redcode", "secret") }),
        ],
        { concurrency: "unbounded" },
      )
  ...
  itKitSecret.live("respects configured basic auth username", () =>
    Effect.gen(function* () {
      const [defaultUser, configuredUser] = yield* Effect.all(
        [getProbe({ authorization: basic("redcode", "secret") }), getProbe({ authorization: basic("kit", "secret") })],
        { concurrency: "unbounded" },
      )
  ...
  itSecret.live("accepts auth token query credentials", () =>
    Effect.gen(function* () {
      const response = yield* HttpClient.get(`/probe?auth_token=${encodeURIComponent(token("redcode", "secret"))}`)
  ...
  itSecret.live("prefers auth token query credentials over basic auth", () =>
    Effect.gen(function* () {
      const response = yield* HttpClientRequest.get(
        `/probe?auth_token=${encodeURIComponent(token("redcode", "secret"))}`,
      ).pipe(HttpClientRequest.setHeader("authorization", basic("redcode", "wrong")), HttpClient.execute)
  ...
  itSecret.live("preserves handler errors when basic auth succeeds", () =>
    Effect.gen(function* () {
      const response = yield* HttpClientRequest.get("/missing").pipe(
        HttpClientRequest.setHeader("authorization", basic("redcode", "secret")),
        HttpClient.execute,
      )
  ...
  itSecret.live("preserves handler errors when auth token query succeeds", () =>
    Effect.gen(function* () {
      const response = yield* HttpClient.get(`/missing?auth_token=${encodeURIComponent(token("redcode", "secret"))}`)
