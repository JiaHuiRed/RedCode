...
//
// Configuration flows through redcode's built-in test affordances:
//   - REDCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - REDCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - REDCODE_DISABLE_PROJECT_CONFIG : skip walking up for redcode.json
...
//
// Today only `redcode.run` is fully wired. The shape supports adding more
// builders (`redcode.serve(opts)`, `redcode.acp(opts)`, `redcode.auth(...)`)
...
const redcodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(redcodeRoot, "src/index.ts")
...
// Typed equivalent of constructing argv for `redcode run`. New flags should
...
// `redcode serve` is a long-lived process — it never exits on its own.
...
// `redcode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `redcode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `redcode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("redcode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("redcode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "redcode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `redcode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("redcode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const redcode: OpencodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, redcode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `redcode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "redcode") {
...
// every call site. Use this for any test that needs the redcode CLI fixture.
//
...
// resources (e.g. `redcode.serve`) without an extra `Effect.scoped` wrapper —
