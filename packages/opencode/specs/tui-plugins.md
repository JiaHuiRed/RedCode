...
- Author package entrypoint is `@redcode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://redcode.dev/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@redcode-ai/plugin/tui`.
- `@redcode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@redcode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@redcode-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@redcode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"redcode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
