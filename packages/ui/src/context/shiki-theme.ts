...
// 260907 Red 从 marked.tsx 抽出：markdown 主线程高亮（getSharedHighlighter）与
// highlight-worker.ts（worker 内 createHighlighter）需要同一份 RedCode 主题。
// 颜色全是 CSS 变量——同一份高亮 HTML 跨主题复用，不用随主题切换重新高亮，
// 这也是它同时喂两条路径而输出保持逐字节一致的前提。
export function redcodeShikiTheme(): ThemeRegistrationResolved {
  return {
    name: "RedCode",
    colors: {
