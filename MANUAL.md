...
**TUI 里直接 `/busy-enter` 切换**（命令面板同样可搜"繁忙时消息"），走 API 写回 `redcode.jsonc` 并强制重载配置，**立即生效不用重启**；手改配置文件则需重启。GUI 设置面板的开关在计划中。TUI 消息列表里繁忙期发送的消息会带 `QUEUED` 徽标；注意 steer 模式下徽标虽在，消息其实很快就送达了（界面状态细化在计划中）。
...
| `/busy-enter` | 切换繁忙时消息送达：插话 ↔ 排队（仅 TUI 内置，立即生效，见 [7.3](#73-繁忙时消息送达busy_enter)）|
