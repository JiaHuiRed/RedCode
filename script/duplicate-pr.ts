...
import { createOpencode } from "@redcode-ai/sdk"
...
`)
  ...
  const redcode = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await redcode.client.session.create()
    const result = await redcode.client.session
      .prompt({
