  ...
  return new Promise((resolve, reject) => {
    ...
    server.on("error", (err) => {
      if (err.code === "EADDRINUSE" && retries > 0) {
        ...
        try {
          execSync(
            `netstat -ano | findstr ":${port}" | findstr "LISTENING" >nul 2>nul && for /f "tokens=5" %a in ('netstat -ano ^| findstr ":${port}" ^| findstr "LISTENING"') do taskkill /F /PID %a 2>nul`,
...
server.tool(
  ...
  { element: z.string(), text: z.string() },
  async ({ element, text }) => {
...
server.tool(
  ...
  { element: z.string(), value: z.string() },
  async ({ element, value }) => {
