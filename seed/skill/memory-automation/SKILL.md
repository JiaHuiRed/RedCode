...
> **260828 存量旧格式就地索引化**：索引化（260812）之前建立的项目，`.redcode/MEMORY.md` 正文是**完整段落全文**（无 `- #NN` 索引行、无"主题+关键动作词"结构），视为旧格式。这类项目**不做一次性迁移**——在该工作区整理记忆（consolidate-memory）或收尾时发现旧格式，就地转换：每条教训全文 INSERT `supermemory.db`（project=项目名，content 首行 `[项目名·踩坑|决策] 标题（YYMMDD）`）→ 正文改写成索引行（4-8 字主题 + 关键动作词）→ 进度段落按旧规归档。新项目不受影响：播种模板已带索引化指引，开局即知机制。
